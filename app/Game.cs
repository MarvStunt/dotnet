...
public partial class Game : Control
{
    ...
    public void OnPlayerSubmitted(string playerName, bool isCorrect, int pointsEarned, int totalScore)
    {
        GD.Print($"Game: {playerName} submitted - {isCorrect} (+{pointsEarned}pts, total: {totalScore})");
        if (isCorrect)
        {
            labelInfo.Text = $"✅ {playerName}: Correct! +{pointsEarned}pts (Total: {totalScore})";
        }
        else
        {
